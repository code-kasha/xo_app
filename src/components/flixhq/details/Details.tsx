import { Details as MovieDetails } from "../../../types/movies/flixhq"

import { Link } from "react-router-dom"

export default function Details({ details, title }: { details: MovieDetails; title: string }) {
	const id = details?.id

	return (
		<>
			{details.cover && <img src={details.cover} className="w-full object-cover" alt={title} />}
			{!details.id ? (
				...
				<>
					...
					{details.episodes && details.episodes[0]?.id && <Link to={`/flixhq/watch?eId=${details.episodes[0].id}&id=${id}`}>Watch</Link>}
				</>
			)}
		</>
	)
